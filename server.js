...
io.on('connection', socket=>{
  ...
  function ensureAbility(room,p,ranks){
    let a=room.abilityPending;
    if(!a && room.drawn[p.id] && specialRanks.has(room.drawn[p.id].r)) { beginAbility(room,p,room.drawn[p.id]); a=room.abilityPending; }
    if(!a || a.ownerId!==p.id || !ranks.includes(a.rank)) throw new Error(`Choose the ${ranks.join(', ')} ability first.`);
    return a;
  }
  socket.on('abilityLookOwn',({index}={})=>{try{const room=roomForSocket(socket),p=ensureTurn(room,socket),a=ensureAbility(room,p,['8','9','10']);if(!Number.isInteger(index)||index<0||index>=p.hand.length)throw new Error('Invalid card position.');log(room,`${p.name} used ${a.rank} to look at position ${index+1}.`);emitAction(room,{type:'ability',actorId:p.id,actorName:p.name,ability:a.rank,status:`${p.name} Used ${a.rank} Ability`});a.stage='reveal';socket.emit('reveal',{kind:'own',cards:[{owner:p.name,index,card:cardPublic(p.hand[index])}],seconds:4});emitRoom(room);setTimeout(()=>{if(!room.ended&&room.turn===playerIndex(room,p.id)){room.abilityPending=null;finishHumanAction(room);}},4050);}catch(e){socket.emit('errorMsg',e.message)}});
  socket.on('abilityJack',({ownIndex,targetId,targetIndex}={})=>{try{const room=roomForSocket(socket),p=ensureTurn(room,socket),a=ensureAbility(room,p,['J']),o=playerById(room,targetId);if(!o||o===p||ownIndex<0||targetIndex<0||ownIndex>=p.hand.length||targetIndex>=o.hand.length)throw new Error('Invalid card position.');emitAction(room,{type:'ability',actorId:p.id,actorName:p.name,ability:'J',status:`${p.name} Used Jack Ability`});[p.hand[ownIndex],o.hand[targetIndex]]=[o.hand[targetIndex],p.hand[ownIndex]];log(room,`${p.name} used Jack to blindly switch with ${o.name}.`);room.abilityPending=null;finishHumanAction(room);}catch(e){socket.emit('errorMsg',e.message)}});
  socket.on('abilityViewPair',({ownIndex,targetId,targetIndex}={})=>{try{const room=roomForSocket(socket),p=ensureTurn(room,socket),a=ensureAbility(room,p,['Q','K']),o=playerById(room,targetId);if(!o||o===p||ownIndex<0||targetIndex<0||ownIndex>=p.hand.length||targetIndex>=o.hand.length)throw new Error('Invalid card position.');emitAction(room,{type:'ability',actorId:p.id,actorName:p.name,ability:a.rank,status:`${p.name} Used ${a.rank} Ability`});a.stage='reveal';room.pendingKing=a.rank==='K'?{ownerId:p.id,targetId:o.id,ownIndex,targetIndex}:null;log(room,`${p.name} used ${a.rank} to inspect two cards.`);socket.emit('reveal',{kind:a.rank==='K'?'king':'queen',cards:[{owner:p.name,index:ownIndex,card:cardPublic(p.hand[ownIndex])},{owner:o.name,index:targetIndex,card:cardPublic(o.hand[targetIndex])}],seconds:4,canSwitch:a.rank==='K'});emitRoom(room);setTimeout(()=>{if(room.ended||room.turn!==playerIndex(room,p.id))return;if(a.rank==='Q'){room.pendingKing=null;room.abilityPending=null;finishHumanAction(room);}},4050);}catch(e){socket.emit('errorMsg',e.message)}});
