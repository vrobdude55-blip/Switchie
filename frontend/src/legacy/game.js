...
const DEMO_MODE=new URLSearchParams(location.search).get('demo')==='1' || window.__SWITCHIE_STATIC_DEMO__===true;
import { createAnimations } from '../animations/gameAnimations.js';
const socket=window.io(window.__SWITCHIE_SOCKET_URL__ || undefined,{autoConnect:true});
...
let valuesPanelCollapsed=false;
...
function card(c,back=false,cls=''){if(back||!c)return `<div class="card back ${cls}"><img class="cardBackImg" src="/cards/card_back_print.png" alt="" draggable="false"></div>`;const print=['J','Q','K'].includes(c.r)?`<img class="facePrint" src="/cards/${c.r.toLowerCase()}_art.png" alt="${c.r} card portrait">`:'';return `<div class="card face ${isRed(c.s)?'red':''} ${['J','Q','K'].includes(c.r)?'printedFace':''} ${cls}"><div class="corner">${esc(c.r)}<br>${suit(c.s)}</div>${print}<div class="suit">${suit(c.s)}</div><div class="corner rot">${esc(c.r)}<br>${suit(c.s)}</div></div>`}
...
function renderDeck(){
  ...
  const deckLayers=Math.max(1,Math.min(6,Math.ceil(count/10)));
  $('deck').innerHTML=Array.from({length:deckLayers},()=>card(null,true)).join('');
  $('deckCount').textContent=state?.started?`${count} cards left`:'52 cards ready';
  // Keep the discard pile visibly thick while keeping the actual top card face-up.
  $('pile').innerHTML=state?.pileTop?Array.from({length:4},()=>card(state.pileTop)).join(''):card(null,true);
...
function renderDrawTimer(){
  ...
  const throwActive=!!(state?.throwOpen&&state?.throwUntil);
  const drawActive=!!(state?.drawPendingPlayerId&&state?.drawDeadline&&state?.drawPhase);
  if(!drawActive&&!throwActive){el.classList.remove('show','waiting','decision','urgent','throwWindow');return}
  el.classList.add('show');
  el.classList.toggle('waiting',drawActive&&state.drawPhase==='waiting');
  el.classList.toggle('decision',drawActive&&state.drawPhase==='decision');
  el.classList.toggle('throwWindow',throwActive&&!drawActive);
  const tick=()=>{
    let remain=0;
    if(throwActive){
      remain=Math.max(0,state.throwUntil-Date.now());
      $('drawTimerNum').textContent=String(Math.ceil(remain/1000));
      $('turnTitle').textContent='Your turn';
      $('drawTimerLabel').textContent='Throw your cards now.';
    }else{
      remain=Math.max(0,state.drawDeadline-Date.now());
      const secs=Math.ceil(remain/1000);
      $('drawTimerNum').textContent=String(secs);
      const minePending=state.drawPendingPlayerId===state.playerId;
      $('turnTitle').textContent = minePending ? 'Your turn' : `${activeName()}'s turn`;
      $('drawTimerLabel').textContent = state.drawPhase==='waiting' ? (minePending?'Draw a card, take an action or knock.':`${activeName()} is drawing a card.`) : (minePending?'Draw a card, take an action or knock.':`${activeName()} is deciding.`);
    }
    const secs=Number($('drawTimerNum').textContent)||0;
...
function renderCenterAction(){
  ...
  if(drawn && ['drawnMenu','pickReplace','pickOwnForAbility','pickOpponent','pickOppCard'].includes(mode)){
    ...
    const buttons=mode==='drawnMenu'?`<div class="caButtons">${special?'<button id="caAbility" class="goldBtn">★ Use Ability</button>':''}<button id="caReplace">⇄ Replace Card</button><button id="caDiscard" class="primary">▢ Discard</button></div>`:'';
...
function render(){
  if(!state)return;
  if(window.showEditorButton)window.showEditorButton();
  $('lobby').style.display='none';$('game').style.display='block';$('roomCode').textContent=state.code;
  $('statusPill').textContent=statusText();
  const me=mine(); $('youTitle').textContent=me?.name||'Player';
  $('roomTitle').textContent=state.title&&state.title!==state.code?state.title:'Private Table';
  renderOpponents();renderDeck();renderHand();renderAbility();renderLog();renderChat();renderCenterAction();renderDrawTimer();
...
animations=createAnimations({ $, card, getState:()=>state, getThrowCardIndex:()=>throwCardIndex, getMode:()=>mode, setMode:v=>{mode=v}, render, showModal, closeModal, socket, esc });
function howToPlay(){
...
function toggleValuesPanel(){
  valuesPanelCollapsed=!valuesPanelCollapsed;
  const panel=$('valuesPanel'),btn=$('valuesMinBtn');
  if(!panel||!btn)return;
  panel.classList.toggle('collapsed',valuesPanelCollapsed);
  btn.textContent=valuesPanelCollapsed?'＋':'−';
  btn.setAttribute('aria-expanded',String(!valuesPanelCollapsed));
  btn.setAttribute('aria-label',valuesPanelCollapsed?'Expand Card Values and Special Cards':'Minimize Card Values and Special Cards');
}
function roomSettings(){
  const host=state?.playerId===state?.hostId;
  showModal(`<div class="modal"><button id="roomSettingsClose" class="modalClose" type="button" aria-label="Close Room Settings">×</button><div class="modalTitle">Room Settings</div><div class="modalSub">${host?'Edit your private table before the game starts.':'Only the host can edit room settings.'}</div><div style="display:grid;gap:8px"><input id="roomTitleInput" class="field" maxlength="32" value="${esc(state?.title&&state.title!==state.code?state.title:'Private Table')}" placeholder="Room name"><button id="saveRoomSettings" class="goldBtn" ${host&&!state.started?'':'disabled'}>Save Room Settings</button><button id="settingsAddAi" ${host&&!state.started?'':'disabled'}>Add AI Player</button><button id="settingsStart" class="primary" ${host&&!state.started&&players().length>=2?'':'disabled'}>Start Game</button></div></div>`);
  if($('roomSettingsClose'))$('roomSettingsClose').onclick=()=>closeModal();
...
$('roomMenuToggle').onclick=()=>{$('roomMenu').classList.toggle('open');$('roomMenuToggle').setAttribute('aria-expanded',String($('roomMenu').classList.contains('open')))};
...
$('valuesMinBtn').onclick=toggleValuesPanel;
...
socket.on('state',x=>{
  ...
  if(state.started&&state.drawn&&next!==lastDrawnKey&&myTurn()){
    ...
    setTimeout(()=>{if(state?.drawn&&state.drawn.r+state.drawn.s===next)animations.animateDraw()},90);
  }else{
    if(!state.drawn)lastDrawnKey='';
    render();
  }
});
socket.on('gameAction',a=>{ if(a?.status){actionStatus=a.status;clearTimeout(actionStatusTimer);actionStatusTimer=setTimeout(()=>{actionStatus='';render()},2600)} animations.animateRemoteAction(a); render(); });
...
socket.on('reveal',x=>animations.revealCard(x.kind==='own'?'Your card':x.kind==='king'?'King · cards revealed':'Queen · cards revealed',x.cards,!!x.canSwitch));
...
if(DEMO_MODE){
  state={
    code:'5B7759',title:'Private Table',hostId:'demo-me',started:true,turn:0,knocked:false,finalTurns:0,ended:false,winner:null,
    pileTop:{r:'K',s:'H'},deckCount:37,drawn:null,players:[
      {id:'demo-me',name:'Nico',ai:false,connected:true,handCount:4},
      {id:'demo-maya',name:'Maya',ai:true,connected:true,handCount:2}
    ],logs:['Game started.','Maya used King and switched two inspected cards.','Maya drew a card.','Nico replaced position 3.','Maya correctly threw 9H.','Nico used 8 to look at position 2.','Nico drew a card.'],scores:null,pendingKing:null,
    throwOpen:false,throwUntil:0,drawPendingPlayerId:'demo-me',drawPhase:'waiting',drawDeadline:Date.now()+15000,abilityPending:null,playerId:'demo-me',
    chat:[{id:'1',from:'Maya',text:'Good luck!',time:new Date().toISOString(),kind:'room'}]
  };
  render();
}
